...
import { 
  ...
  ChevronRight, Calendar, Globe, Shield, Menu, X, Zap, Image as ImageIcon, ListOrdered, Table as TableIcon
} from 'lucide-react';
import { useTheme } from '@/app/providers/ThemeContext';
import { TimelineView, AttributesView, GalleryView, QuoteView, OrderedListView, TableView, CategoryHeader } from '@/features/content/components/wiki/WikiComponents';
...
export const WikiPage: React.FC = () => {
  ...
  const filteredCategories = useMemo(() => {
    ...
    Object.entries(categories).forEach(([cat, articles]) => {
      const matching = (articles as WikiArticle[]).filter(a => a.title.toLowerCase().includes(searchQuery.toLowerCase()));
    ...
    <div className="min-h-screen bg-warm-50 dark:bg-warm-950 flex flex-col md:flex-row pb-[env(safe-area-inset-bottom)]">
      ...
      <div className="md:hidden sticky top-0 z-40 bg-white/90 dark:bg-warm-900/90 backdrop-blur-md border-b border-warm-200 dark:border-warm-800 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Button onClick={() => navigate(-1)} variant="ghost" size="icon" className="-ml-2">
            <ArrowLeft size={20} />
          </Button>
          <span className="font-serif font-bold text-lg text-warm-900 dark:text-warm-100">Archives</span>
        </div>
        <Button onClick={() => setSidebarOpen(true)} variant="secondary" size="icon">
          <Menu size={20} />
        </Button>
      </div>
        ...
        <div className="h-full flex flex-col">
          <div className="p-6 pb-2 border-b border-warm-100 dark:border-warm-800 bg-warm-50/50 dark:bg-warm-900">
            <div className="flex items-center justify-between mb-6 md:mb-4">
               <Button onClick={() => navigate(-1)} variant="ghost" size="sm" icon={<ArrowLeft size={16} />} className="hidden md:flex">
                  Back to App
               </Button>
               <Button onClick={() => setSidebarOpen(false)} variant="ghost" size="icon" className="md:hidden rounded-full">
                 <X size={20} />
               </Button>
            </div>
